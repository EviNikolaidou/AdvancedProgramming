...
public class RegistrationCommand implements Command {
	...
	public RegistrationCommand(BufferedReader in, BufferedWriter out, MsgSvrConnection serverConn) {
		this.in = in;
		this.out = out;
	...
	public void execute() throws IOException {
		if (conn.getCurrentUser() != null) {
			(new ErrorCommand(in, out, conn, "Error - cannot create new account as you are already logged in"))
					.execute();
		} else {
			out.write("Please enter your desired username: ");
			out.flush();
			String user = in.readLine();
			if (user.isEmpty()) {
				(new ErrorCommand(in, out, conn, "Error - You did not enter a username")).execute();
			} else {
				out.write("Please enter your desired password: ");
				out.flush();
				String password = in.readLine();
				if (password.isEmpty()) {
					(new ErrorCommand(in, out, conn, "Error - You did not enter a password")).execute();
				} else {
					out.write("Would you like to fully register? ('y' or 'n'): ");
					out.flush();
						...
						if (yesOrNo.equals("y")) {
							...
							if (day > 0 && day <= 31) {
								...
								if ((month.equals("Jan") && day <= 31) || (month.equals("Feb") && day <= 29)
									...
									if (year > 2017) {
										/*
										 * if the year was after 2017 then it is
										 * not a valid year to be born in as you
										 * can't be born in the future
										 */
										(new ErrorCommand(in, out, conn, "Error - " + year + " is in the future."))
												.execute();
									} else if (year < 1900) {
										...
										/*
										 * Checks that the year entered is valid
										 * If the year entered is 'Feb' and a
										 * leap year then any day entered below
										 * 30 is valid If the year entered is
										 * 'Feb' and is not a leap year then any
										 * day below 29 is valid
										 */
										out.write("You entered a valid date \r\n");
										out.flush();
										/*
										 * The code below writes the username
										 * and password to the "pwd.txt" file
										 */
										...
										/*
										 * The code below writes the username,
										 * password, phone number, date of birth
										 * and address details to the
										...
										out.write(
												"User created - Please re-open the program to log in using a new user\r\n");
										out.write("200\r\n");
										out.flush();
										break;
									} else if (isLeapYear == false && month.equals("Feb") && day == 29) {
										/*
										 * if the year was valid but the 29th of
										 * Feb was entered and it wasn't a leap
										 * year then an error message is
										 * returned stating that it's not a leap
										 * year
										 */
										(new ErrorCommand(in, out, conn, "Error - " + year + " is not a leap year."))
												.execute();
									} else {
										/*
										 * if the year entered does not apply to
										 * previous conditions and was invalid a
										 * generic error statement is returned
										 */
									...
									/*
									 * if month entered was valid but the day
									 * was invalid (e.g. the 31st of April) then
									...
									/*
									 * if the month entered was not a recognised
									 * month an error message is returned
									 */
									(new ErrorCommand(in, out, conn,
											"Error - " + month
								...
								/*
								 * returns an error message if day entered is
								 * outside of the limits 1 - 31
								 */
								(new ErrorCommand(in, out, conn, "You entered a day outside of the limits (1-31)."))
										.execute();
							}
						} else if (yesOrNo.equals("n")) {
							/*
							 * The code below writes the username and password
							 * to the "pwd.txt" file
							 */
							try (FileWriter fw = new FileWriter("pwd.txt", true);
									BufferedWriter bw = new BufferedWriter(fw);
							...
							out.write("User created - Please re-open the program to log in using a new user\r\n");
							out.write("200\r\n");
							/*
							 * the above line is printed so that the assessor
							 * can see that it actually works
							 */
							out.flush();
							/*
							 * the problem with the above code is that it
							 * requires the user to terminate and re-open
							 * program for it to work, which needs fixing
							 */
							break;
						} else {
